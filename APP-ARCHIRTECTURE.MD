...
2.  **Window Creation (`createWindow` in `src/main.ts`):**
    ...
    *   Crucially, it specifies the `preload` script: `preload: path.join(__dirname, 'preload.js')`.
...
3.  **Preload Script Execution (`src/preload.ts`):**
    *   This script runs in a privileged context before the renderer's web content begins loading but after the DOM is available.
    *   It uses `contextBridge.exposeInMainWorld('electronAPI', ...)` to securely expose specific Electron/Node.js functionalities to the renderer process. This includes:
...
4.  **Renderer Process Initialization (`src/renderer.ts`):**
    *   This is the entry point for the Vite-bundled frontend code.
    *   It imports the main SCSS file (`./index.scss`) for global styles.
    *   It imports and executes `./app/App.tsx`.
...
*   **ATC Slice (`src/app/store/atc/atsSlice.ts`):**
...
*   **Context (`src/app/context/MusicContext.tsx`):**
    ...
    *   Provides state like `videoInfo`, `isPlayng`, `isBuffering`, `volume`.
