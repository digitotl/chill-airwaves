...
- **`atcProtocol.ts` (`src/protocols/`):** Implements the custom `atc://` protocol handler. It now directly proxies requests for ATC audio to the configured `CLOUDFLARE_CDN_URL`.
